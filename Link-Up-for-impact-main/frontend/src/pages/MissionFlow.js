...
const MissionFlow = () => {
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(swipeDistance) > minSwipeDistance) {
      if (swipeDistance > 0 && phase < 3) {
        if (phase === 1 && fase1Data.crenca_limitante && fase1Data.previsao_catastrofica) {
          setPhase(2);
        } else if (phase === 2) {
          setPhase(3);
        }
      } else if (swipeDistance < 0 && phase > 1) {
        setPhase(phase - 1);
      }
    }
  };
  ...
  return (
    <div className="App" data-testid="mission-flow" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      <BackgroundLines />
      ...
      <div style={{maxWidth: '900px', margin: '0 auto', padding: 'clamp(1rem, 4vw, 2rem)', position: 'relative', zIndex: 10}}>
        ...
        <p style={{textAlign: 'center', fontSize: '0.75rem', color: 'var(--texto-corpo)', marginTop: '1rem'}}>
          Deslize para navegar entre fases
        </p>
      </div>
